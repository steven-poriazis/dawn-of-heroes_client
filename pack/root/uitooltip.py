...
class ItemToolTip(ToolTip):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, window_type = player.INVENTORY, slotIndex = -1, transmutation = -1):
		...
		if app.ENABLE_ACHIEVEMENT:
			if itemType == item.ITEM_TYPE_ACHIEVEMENT and itemSubType == item.ACHIEVEMENT_TITLE:
				if 0 != metinSlot:
					achievementName, r, g, b = player.GetAchievementInfo(metinSlot[0], metinSlot[1])
					name = item.GetItemName()
					if metinSlot[0] >= 0:
						name += " - "
						name += achievementName
					self.AppendTextLine(name, grp.GenerateColor(r, g, b, 1.0))
					itemDesc = item.GetItemDescription()
					itemSummary = item.GetItemSummary()
					self.AppendDescription(itemDesc, 26)
					self.AppendDescription(itemSummary, 26, self.CONDITION_COLOR)
					self.ShowToolTip()
				return
			elif itemType == item.ITEM_TYPE_ACHIEVEMENT and itemSubType == item.ACHIEVEMENT_COLOR:
				if 0 != metinSlot:
					tmpName, r, g, b = player.GetAchievementInfo(0, metinSlot[0])
					self.AppendTextLine(item.GetItemName(), grp.GenerateColor(r, g, b, 1.0))
					itemDesc = item.GetItemDescription()
					itemSummary = item.GetItemSummary()
					self.AppendDescription(itemDesc, 26)
					self.AppendDescription(itemSummary, 26, self.CONDITION_COLOR)
	...
	def __IsNewHair3(self, itemVnum):
		return ((74012 < itemVnum and itemVnum < 74022) or
			...
			(45000 < itemVnum and itemVnum < 45999))
		...
		def AppendTransmutation(self, window_type, slotIndex, transmutation):
			...
			if item.GetItemType() == item.ITEM_TYPE_ARMOR:
				if item.GetItemSubType() == item.ARMOR_BODY:
					...
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_WARRIOR) and item.IsAntiFlag(item.ITEM_ANTIFLAG_SHAMAN) and item.IsAntiFlag(item.ITEM_ANTIFLAG_SURA):
						malefemale = "Ninja"		

					if item.IsAntiFlag(item.ITEM_ANTIFLAG_WARRIOR) and item.IsAntiFlag(item.ITEM_ANTIFLAG_SHAMAN) and item.IsAntiFlag(item.ITEM_ANTIFLAG_ASSASSIN):
						malefemale = "Sura"							

					if item.IsAntiFlag(item.ITEM_ANTIFLAG_WARRIOR) and item.IsAntiFlag(item.ITEM_ANTIFLAG_SURA) and item.IsAntiFlag(item.ITEM_ANTIFLAG_ASSASSIN):
						malefemale = "Shaman"
					itemName += " ( " + malefemale +  " )"						
			...
			self.AppendTextLine(title, self.CHANGELOOK_TITLE_COLOR)
