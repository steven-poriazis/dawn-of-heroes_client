...
class GameButtonWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self, filename):
		...
		try:
			...
			self.GetChild("AchievementButton").SetEvent(ui.__mem_func__(self.__ClickOpenAchievementButton))
	...
	def __ClickOpenAchievementButton(self):
		if app.ENABLE_ACHIEVEMENT:
			if self.interface:
				self.interface.OpenAchievementWindow()
