...
class LoadingWindow(ui.ScriptWindow):
	...
	if app.ENABLE_ACHIEVEMENT:
		def __RegisterAchievement(self):
			id = 0
			for achievement in localeInfo.ACHIEVEMENT_NAME_LIST:
				# split
				try:
					splitPos = achievement.find("/")
					while splitPos != -1 and achievement[splitPos-1:splitPos] == '\\':
						newSplitPos = achievement[splitPos+1:].find("/")
						if newSplitPos == -1:
							splitPos = -1
						else:
							splitPos += newSplitPos + 1

					name_m = achievement[:achievement.find("|")]
					name_w = name_m[splitPos+1:]
					if name_m != name_w:
						name_m = name_m[:splitPos]

					name_m = name_m.replace("\\/", "/")
					name_w = name_w.replace("\\/", "/")

					color = achievement[achievement.find("|")+1:]
					r = ()
					g = ()
					b = ()
					for i in xrange(player.ACHIEVEMENT_MAX_COLOR):
						r += (float(color[:color.find(",")]),)
						color = color[color.find(",")+1:]
						g += (float(color[:color.find(",")]),)
						color = color[color.find(",")+1:]
						b += (float(color[:color.find(";")]),)
						color = color[color.find(";")+1:]
				except:
					import sys
					import dbg
					dbg.TraceError("invalid achievement (index %i) %s" % (id, str(achievement)))#,str(achievement)))# str(sys.exc_info()[0])))
					continue
				# register
				player.RegisterAchievementTitle(id, name_m, name_w, r, g, b)
				# increase id
				id += 1
