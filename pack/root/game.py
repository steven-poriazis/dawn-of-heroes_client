...
class GameWindow(ui.ScriptWindow):
	def __init__(self, stream):
		...
		if app.ENABLE_ACHIEVEMENT:
			self.achievementRewardShower = None
		...
		if app.ENABLE_ACHIEVEMENT:
			self.achievementRewardShower = uiAchievement.AchievementRewardShower()	
	...
	def OpenQuestWindow(self, skin, idx):
		if work_chriss.FIX_INPUT_VALUE:
			if work_chriss.INPUT_IGNORE == 1:
				return
			else:
	...
	def RecvPartyInviteQuestion(self, leaderVID, leaderName):
		if work_chriss.REQUEST_AUTO_CANCEL_BY_OVERTIME:
			partyInviteQuestionDialog = uiCommon.QuestionDialogWithTimeLimitTHL()
	...
	def OnMessengerAddFriendQuestion(self, name):
		if work_chriss.REQUEST_AUTO_CANCEL_BY_OVERTIME:
			messengerAddFriendQuestion = uiCommon.QuestionDialogWithTimeLimitTHL()
			messengerAddFriendQuestion.SetText1(localeInfo.MESSENGER_DO_YOU_ACCEPT_ADD_FRIEND_THL % (name))
	...
	def __ServerCommand_Build(self):
		...
		if work_chriss.FIX_INPUT_VALUE:
			self.serverCommander.SAFE_RegisterCallBack("getinputbegin", self.__Inputget1)
			self.serverCommander.SAFE_RegisterCallBack("getinputend", self.__Inputget2)
			
		if app.ENABLE_BIOLOG_SYSTEM:
			self.serverCommander.SAFE_RegisterCallBack("BINARY_Biolog_Update", self.BINARY_Biolog_Update)
			self.serverCommander.SAFE_RegisterCallBack("BINARY_Biolog_SendMessage", self.BINARY_Biolog_SendMessage)	
			self.serverCommander.SAFE_RegisterCallBack("BINARY_Biolog_PopUp", self.BINARY_Biolog_PopUp)	
			self.serverCommander.SAFE_RegisterCallBack("BINARY_Biolog_SelectReward", self.BINARY_Biolog_SelectReward)	
	...
	if app.ENABLE_ACHIEVEMENT:
		def BINARY_Achievement_Load(self):
			self.interface.LoadAchievement()

		def BINARY_Achievement_Update(self, index):
			self.interface.UpdateAchievement(index)
			if player.HasAchievement(index):
				self.achievementRewardShower.Open(index)

		def BINARY_Achievement_RefreshCurrent(self):
			self.interface.RefreshCurrentAchievement()
	...
	if work_chriss.FIX_INPUT_VALUE:
		def __Inputget1(self):
			work_chriss.INPUT_IGNORE = 1 

		def __Inputget2(self):
			work_chriss.INPUT_IGNORE = 0
