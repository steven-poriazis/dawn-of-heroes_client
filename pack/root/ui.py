...
class Window(object):
	...
	def __init__(self, layer = "UI"):
		...
		self.fadeType = 0
		self.fadingStartAlpha = 0.0
		self.fadingStartTime = -1
		self.fadingEndTime = 0
		self.fadingFinishEvent = None
		self.fadingFinishArgs = None
		self.fadeIsSplash = FALSE
		self.fadingSplashEvent = None
		self.fadingSplashArgs = None

		self.mouseLeftButtonDownEvent = None
		self.mouseLeftButtonUpEvent = None
		self.mouseOverInEvent = None
		self.mouseOverOutEvent = None
		self.finishRenderEvent = None
	...
	def SetFadeFinishEvent(self, event, *args):
		self.fadingFinishEvent = event
		self.fadingFinishArgs = args

	def SetSplashFinishEvent(self, event, *args):
		self.fadingSplashEvent = event
		self.fadingSplashArgs = args

	def FadeIn(self, fadingTime, startTime=0, reset=FALSE, isSplash=FALSE):
		if reset == TRUE:
			self.SetAllAlpha(0.0)

		self.Fade(1.0, fadingTime, startTime, isSplash)

	def FadeOut(self, fadingTime, startTime=0, reset=FALSE, isSplash=FALSE):
		if reset == TRUE:
			self.SetAllAlpha(1.0)

		self.Fade(0.0, fadingTime, startTime, isSplash)

	def Fade(self, targetAlpha, fadingTime, startTime=0, isSplash=FALSE):
		self.fadeIsSplash = isSplash

		self.fadingStartAlpha = self.GetAlpha()
		self.fadingEndAlpha = targetAlpha
		self.fadingStartTime = app.GetTime() + startTime
		self.fadingEndTime = app.GetTime() + startTime + fadingTime

		self.Show()

	def Splash(self, startAlpha, endAlpha, fadeInTime, fadeOutTime, fadeInWaitTime=0, fadeOutWaitTime=0):
		self.fadeIsSplash = TRUE
		self.SetAllAlpha(startAlpha)

		self.SetFadeFinishEvent(self.Fade, startAlpha, fadeOutTime, fadeOutWaitTime, TRUE)
		self.Fade(endAlpha, fadeInTime, fadeInWaitTime, TRUE)

	def IsFading(self):
		return self.fadingStartTime != -1 and app.GetTime() >= self.fadingStartTime
	...
	def OnUpdate(self):
		...
		perc = float(app.GetTime() - self.fadingStartTime) / float(self.fadingEndTime - self.fadingStartTime)

		if perc >= 1.0:
			self.StopFading()
			self.SetAllAlpha(self.fadingEndAlpha)
			if self.fadingEndAlpha > 0.0:
				self.Show()
			else:
				self.Hide()
			if self.fadingFinishEvent:
				fadingFinishEvent = self.fadingFinishEvent
				fadingFinishArgs = self.fadingFinishArgs
				self.fadingFinishEvent = None
				self.fadingFinishArgs = None

				apply(fadingFinishEvent, fadingFinishArgs)
			elif self.fadeIsSplash == TRUE and not self.IsFading() and self.fadingSplashEvent:
				apply(self.fadingSplashEvent, self.fadingSplashArgs)

			return

		if self.fadingStartAlpha < self.fadingEndAlpha:
			alpha = perc * (self.fadingEndAlpha - self.fadingStartAlpha) + self.fadingStartAlpha
		elif self.fadingEndAlpha < self.fadingStartAlpha:
			alpha = self.fadingStartAlpha - perc * (self.fadingStartAlpha - self.fadingEndAlpha)
		else:
			return

		self.SetAllAlpha(alpha)

	def SetRenderFinishEvent(self, event):
		self.finishRenderEvent = event

	def OnRenderFinish(self):
		if self.finishRenderEvent:
			self.finishRenderEvent()
	...
	if app.ENABLE_SEND_TARGET_INFO or app.ENABLE_MAINTENANCE_SYSTEM:
		def GetLeft(self):
...
class TitleBar(Window):
	...
	def MakeTitleBar(self, width, color):
		...
		btnClose.Show()
		btnInfo = Button()
		btnInfo.SetParent(self)
		btnInfo.SetUpVisual("%s/%s" % (self.IMAGES['INFO']['PATH'], self.IMAGES['INFO']['NORMAL']))
		btnInfo.SetOverVisual("%s/%s" % (self.IMAGES['INFO']['PATH'], self.IMAGES['INFO']['OVER']))
		btnInfo.SetDownVisual("%s/%s" % (self.IMAGES['INFO']['PATH'], self.IMAGES['INFO']['DOWN']))
		btnInfo.Show()
	...
	def SetWidth(self, width, isScale = False):
		if isScale:
			...
			self.imgCenter.SetPosition(self.imgLeft.GetWidth(), 0)
			self.imgCenter.SetScale(float(self.GetWidth() - (self.imgLeft.GetWidth() + self.imgRight.GetWidth())) / float(self.imgCenter.GetWidth()), 1.0)
			self.imgRight.SetPosition(self.imgLeft.GetWidth() + self.imgCenter.GetWidth(), 0)
			self.btnClose.SetPosition(width - self.btnClose.GetWidth() - self.POSITIONS["C"][0], self.POSITIONS["C"][1])
			self.btnInfo.SetPosition(width - self.btnInfo.GetWidth() - self.POSITIONS["Q"][0], self.POSITIONS["Q"][1])
		else:
	...
	def SetInfoToolTip(self, tooltipWnd):
		self.btnInfo.SetToolTipWindow(tooltipWnd)
		
	def InfoButton(self, command):
		if command == "hide":
			self.btnInfo.Hide()
		elif command == "show":
			self.btnInfo.Show()
...
class ListBox(Window):
	...
	def InsertItem(self, number, text, extraTextLineFunc = None, *extraTextLineArgs):
		...
		if extraTextLineFunc:
			apply(extraTextLineFunc, (textLine,) + extraTextLineArgs)
		textLine.Show()
	...
	def GetSelectedItem(self, default = 0):
		return self.keyDict.get(self.selectedLine, default)
...
class PythonScriptLoader(object):
	...
	def LoadDefaultData(self, window, value, parentWindow):
		...
		if value.has_key("all_alpha"):
			window.SetAllAlpha(float(value["all_alpha"]))
